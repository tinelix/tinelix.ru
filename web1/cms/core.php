<?php
    ...
    function genWebsiteMenu($html_encoding) {
        ...
        $new_year_countdown = -round((dt->getTimestamp() - strtotime("2024-01-01")) / (60 * 60 * 24));
        if($new_year_countdown >= 0) {
            $new_year_countdown_h = -(round((dt->getTimestamp() - strtotime("2024-01-01")) / (60 * 60)) % 24);
            $new_year_countdown_min = -(round((dt->getTimestamp() - strtotime("2024-01-01")) / 60) % 60);
            $new_year_countdown_sec = -(round(dt->getTimestamp() - strtotime("2024-01-01")) % 60);
        } else {
	    ...
	    \r\n				<b class=\"highlight\">".str_pad($new_year_countdown_h, 2, "0", STR_PAD_LEFT)."</b> час.
